...
class BookRecommender:
    def __init__(self, api_key="0dfddd5045ff123245cc00ab9034d122d6b6c1e6fba60c838f7304b8f98a69c1", gemini_api_key=None):
    ...
    def verify_relevance_with_gemini(self, books: List[Dict], search_keyword: str) -> List[Dict]:
        """Gemini API를 사용하여 검색 결과와 키워드의 관련성 검증 (필수)"""
        if not books:
            return books
...
"""
            ...
            if "RELEVANT:" in response_text:
                ...
                try:
                    ...
                    # 관련성 높은 도서만 선택 (인덱스는 0부터 시작하므로 -1)
                    ...
                    print(f"⚠️ Gemini 응답 파싱 실패, 모든 자료 제외: {relevant_part}")
                    return []
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if final_books:
                    ...
                    print(f"    (전체 {len(books)}개 → 필터링 {len(filtered_books)}개 → 중복제거 {len(unique_books)}개 → 관련성검증 {len(verified_books)}개)")
                else:
...
def main():
    """메인 실행 함수"""
    parser = argparse.ArgumentParser(description="우선순위 키워드 기반 도서 추천 시스템 (Gemini 검증 필수)")
    ...
    print("🤖 모든 검색 결과는 Gemini API로 관련성 검증을 거칩니다.")
    
    try:
        # Book Recommender 초기화 (Gemini API 키 필수)
        recommender = BookRecommender(gemini_api_key=args.gemini_api_key)
    except ValueError as e:
        print(str(e))
        print("\n사용법:")
        print("python book_recommender.py -i keywords.csv -g YOUR_GEMINI_API_KEY")
        sys.exit(1)
