...
# 시스템 의존성 설치 (Chrome + ChromeDriver 포함)
...
RUN echo '#!/bin/bash' > /app/run_pipeline.sh && \
    ...
    echo '' >> /app/run_pipeline.sh && \
    echo '# 로그 함수 (파일과 stdout 둘 다)' >> /app/run_pipeline.sh && \
    ...
    echo '}' >> /app/run_pipeline.sh && \
    echo '' >> /app/run_pipeline.sh && \
    echo '# 환경변수 확인' >> /app/run_pipeline.sh && \
    echo 'if [ -z "$GEMINI_API_KEY" ]; then' >> /app/run_pipeline.sh && \
    echo '    log "❌ GEMINI_API_KEY 환경변수가 설정되지 않았습니다"' >> /app/run_pipeline.sh && \
    echo '    exit 1' >> /app/run_pipeline.sh && \
    echo 'fi' >> /app/run_pipeline.sh && \
    echo '' >> /app/run_pipeline.sh && \
    echo 'if [ -z "$NAVER_CLIENT_ID" ] || [ -z "$NAVER_CLIENT_SECRET" ]; then' >> /app/run_pipeline.sh && \
    echo '    log "❌ 네이버 API 키가 설정되지 않았습니다"' >> /app/run_pipeline.sh && \
    echo '    exit 1' >> /app/run_pipeline.sh && \
    echo 'fi' >> /app/run_pipeline.sh && \
    echo '' >> /app/run_pipeline.sh && \
    echo 'DATE=$(date "+%y%m%d")' >> /app/run_pipeline.sh && \
    ...
    echo '    --gemini_api_key "$GEMINI_API_KEY" \' >> /app/run_pipeline.sh && \
    ...
    echo 'fi' >> /app/run_pipeline.sh && \
    echo '' >> /app/run_pipeline.sh && \
    echo 'sleep 10  # 단계간 대기' >> /app/run_pipeline.sh && \
    echo '' >> /app/run_pipeline.sh && \
    ...
    echo 'log "   📁 Firebase 저장 경로:"' >> /app/run_pipeline.sh && \
    echo 'log "     - keywords/$DATE"' >> /app/run_pipeline.sh && \
    echo 'log "     - news/$DATE"' >> /app/run_pipeline.sh && \
    echo 'log "     - trend/$DATE"' >> /app/run_pipeline.sh && \
    echo 'log "     - source/$DATE"' >> /app/run_pipeline.sh && \
...
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \
    CMD test -f /app/run_pipeline.sh || exit 1
